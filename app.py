...
def generate():
    if request.method == 'POST':
        ...
        if speech_file and speech_file.filename.endswith('.mp3') and background_file and background_file.filename.endswith('.mp3'):
            speech_file.save(os.path.join(app.config['UPLOAD_FOLDER_speech'], secure_filename(speech_file.filename)))
            background_file.save(os.path.join(app.config['UPLOAD_FOLDER_background'], secure_filename(background_file.filename)))
